...
要点：
- 模块顶部 `logger = logging.getLogger(__name__)`，`__name__` 自动形成层级（`fcmd.core.sub`）。
...
PROD_CONFIG: dict[str, Any] = {
    "version": 1, "disable_existing_loggers": False,
    "formatters": {
        "json": {"()": "fcmd.logging_config.JsonFormatter"},
        "simple": {"format": _SIMPLE_FMT},
    },
    "handlers": {
        ...
        "file": {"class": "logging.handlers.RotatingFileHandler", "level": "INFO",
                  "formatter": "json", "filename": "logs/fcmd.log",
...
def setup_logging(env: str = "dev", log_dir: Path | None = None) -> None:
    ...
    if env == "prod" and log_dir is not None:
        ...
        config["handlers"]["file"]["filename"] = str(log_dir / "fcmd.log")
...
def configure_loggers() -> None:
    ...
    层级：root=ERROR（兜底）→ fcmd=INFO（自研默认）→ 子模块按需放低；
    第三方库（urllib3/asyncio/httpx）一律 WARNING。
    """
    logging.getLogger().setLevel(logging.ERROR)  # root 兜底
    logging.getLogger("fcmd").setLevel(logging.INFO)
    # 调试某模块时动态放低（不改 root）：logging.getLogger("fcmd.db").setLevel(logging.DEBUG)
