...
- 现有 `fcmd` 的 `cli.py` 默认用 argparse；引入 Typer 时在 `pyproject.toml` 加 `typer>=0.12.0` 依赖。
...
@click.group(help="极速 python 工具集。")
@click.version_option(version=__version__, prog_name="fcmd")
def cli() -> None:
    """fcmd 命令组."""
...
from fcmd import cli_db

app = typer.Typer(name="fcmd", help="极速 python 工具集。", no_args_is_help=True)

# 合并子模块命令组：fcmd db migrate / fcmd db backup
...
- 嵌套层级 ≤ 2（`fcmd db migrate`）；超过说明拆分不合理。
...
2. **颜色码污染重定向输出**：未检测终端直接输出 ANSI 码，`fcmd > log.txt` 含乱码。用 `click.echo` 的 `color` 参数或 `click.style` 自动检测。
...
10. **进度条写入 stdout 干扰管道**：`fcmd process | grep` 时进度条混入数据流。进度条用 stderr（`rich.Console(stderr=True)`），数据输出用 stdout。
