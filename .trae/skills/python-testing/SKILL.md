...
from fcmd import validate_non_empty
...
def test_load_config_falls_back_when_env_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    """环境变量缺失时应回退到默认值。"""
    monkeypatch.delenv("FCMD_TIMEOUT", raising=False)
    cfg = config.load_from_env()
...
from fcmd import discover_configs
...
from fcmd import StorageError, load
...
# 全量（含慢测试）
uv run pytest --cov=fcmd --cov-branch --cov-report=term-missing

# HTML 报告（本地排查未覆盖行）
uv run pytest --cov=fcmd --cov-report=html
```
...
from fcmd.windows.main_window import MainWindow
...
def test_main_window_title(qtbot) -> None:
    ...
    assert window.windowTitle() == "fcmd"
...
from fcmd.widgets.search_box import SearchBox
