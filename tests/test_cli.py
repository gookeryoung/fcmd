...
def test_tool_main_delegates_run_tool(tool_name: str, module_path: str, monkeypatch: pytest.MonkeyPatch) -> None:
    """``<tool>.main()`` 通过 ``@fcmd.main`` 委托 ``run_tool`` 并传入正确工具名。

    验证各工具模块的 ``main()`` 入口正确绑定工具名，等价于 ``fcmd <tool> <args>``。
    通过 monkeypatch 替换 ``toolkit.run_tool`` 避免触发真实命令执行与 ``SystemExit``。
    """
    import importlib
...
class TestMainDecorator:
    ...
    def test_main_decorator_returns_callable(self) -> None:
        """装饰器返回可调用对象。"""
        import fcmd as fx
        ...
        assert callable(my_main)

    def test_main_decorator_auto_docstring(self) -> None:
        """装饰器自动生成统一格式的中文文档字符串。"""
        import fcmd as fx
        ...
        expected = "``my_tool`` 入口：等价于 ``fcmd my_tool <args>``。"
        assert my_main.__doc__ == expected
